...
class TrainingPlotter:
    ...
    def plot_training_history(self, history, title="История обучения", save_name="training_history.png", model_type=None):
        """
    ...
    def _plot_autoencoder_history(self, history, title, save_name):
        ...
        axes[0, 0].annotate(f'Min: {min_loss:.4f}', 
        ...
        if 'val_mean_errors' in history and history['val_mean_errors']:
            ...
            if 'val_std_errors' in history and history['val_std_errors']:
                ...
                axes[0, 1].fill_between(val_epochs, 
                                      np.array(mean_errors) - np.array(std_errors),
                                      np.array(mean_errors) + np.array(std_errors),
                                      alpha=0.3, color='green', label='±σ')
            ...
            axes[0, 1].annotate(f'Best: {min_val_error:.4f}', 
                               xy=(min_val_epoch, min_val_error), xytext=(10, -20),
                               textcoords='offset points', fontsize=9,
                               bbox=dict(boxstyle='round,pad=0.3', facecolor='lightgreen', alpha=0.7))
        else:
            axes[0, 1].text(0.5, 0.5, 'Нет данных валидации', 
        ...
        if 'epoch_times' in history and history['epoch_times']:
            ...
            axes[1, 0].text(0.02, 0.98, f'Общее время: {total_time:.1f}с', 
                           transform=axes[1, 0].transAxes, fontsize=9,
                           bbox=dict(boxstyle='round,pad=0.3', facecolor='lightblue', alpha=0.7),
                           verticalalignment='top')
        else:
            axes[1, 0].text(0.5, 0.5, 'Нет данных времени', 
    ...
    def _plot_lstm_history(self, history, title, save_name):
        ...
        if val_losses_key in history and history[val_losses_key]:
            ...
            axes[0, 0].plot(val_epochs, history[val_losses_key], 'r-', label='Валидация', linewidth=2)
            ...
            axes[0, 0].annotate(f'Best: {min_val_loss:.4f}', 
        ...
        if train_accs_key in history and history[train_accs_key]:
            ...
            if val_accs_key in history and history[val_accs_key]:
                ...
                axes[0, 1].plot(val_epochs, history[val_accs_key], 'r-', label='Валидация', linewidth=2)
                ...
                axes[0, 1].annotate(f'Best: {max_val_acc:.3f}', 
                                   xy=(max_val_epoch, max_val_acc), xytext=(10, -20),
                                   textcoords='offset points', fontsize=9,
                                   bbox=dict(boxstyle='round,pad=0.3', facecolor='lightgreen', alpha=0.7))
            ...
            axes[0, 1].text(0.5, 0.5, 'Нет данных точности', 
                           transform=axes[0, 1].transAxes, ha='center', va='center')
            axes[0, 1].set_title('Точность')
    ...
    def _plot_generic_history(self, history, title, save_name):
        ...
        for i in range(3):
            ...
            axes[row, col].text(0.5, 0.5, 'Данные недоступны', 
...
class ResultsPlotter:
    ...
    def plot_score_distributions(self, normal_scores, attack_scores, threshold=None, 
                                title="Распределение оценок", save_name="score_distributions.png"):
        """
        ...
        # Гистограммы
        axes[0, 0].hist(normal_scores, bins=50, alpha=0.7, color='green', label=f'Нормальные (n={len(normal_scores)})')
        axes[0, 0].hist(attack_scores, bins=50, alpha=0.7, color='red', label=f'Атаки (n={len(attack_scores)})')
        if threshold is not None:
            axes[0, 0].axvline(threshold, color='black', linestyle='--', linewidth=2, label=f'Порог: {threshold:.4f}')
...
"""
    ...
    def plot_roc_curve(self, y_true, y_scores, title="ROC кривая", save_name="roc_curve.png"):
        ...
        # ROC кривая
        axes[0].plot(fpr, tpr, color='darkorange', linewidth=3, label=f'ROC кривая (AUC = {roc_auc:.4f})')
        axes[0].plot([0, 1], [0, 1], color='navy', linewidth=2, linestyle='--', label='Случайный классификатор')
        ...
        axes[0].plot(optimal_fpr, optimal_tpr, 'ro', markersize=8, 
        ...
        print(f"🎯 Оптимальный порог: {optimal_threshold:.4f} (TPR={optimal_tpr:.3f}, FPR={optimal_fpr:.3f})")
    ...
    def plot_precision_recall_curve(self, y_true, y_scores, title="Precision-Recall кривая", 
        ...
        # PR кривая
        axes[0].plot(recall, precision, color='blue', linewidth=3, 
                    label=f'PR кривая (AP = {ap_score:.4f})')
        axes[0].fill_between(recall, precision, alpha=0.2, color='blue')
        
        # Baseline (случайный классификатор)
        baseline = np.sum(y_true) / len(y_true)
        axes[0].axhline(baseline, color='red', linestyle='--', linewidth=2,
    ...
    def create_comprehensive_analysis(self, y_true, y_scores, y_pred=None, threshold=None,
                                    title="Комплексный анализ", save_name="comprehensive_analysis.png"):
        ...
        plt.hist(normal_scores, bins=50, alpha=0.7, color='green', label=f'Нормальные (n={len(normal_scores)})', density=True)
        plt.hist(attack_scores, bins=50, alpha=0.7, color='red', label=f'Атаки (n={len(attack_scores)})', density=True)
        if threshold is not None:
            plt.axvline(threshold, color='black', linestyle='--', linewidth=2, label=f'Порог: {threshold:.4f}')
        ...
        if y_pred is not None:
            ...
            report = classification_report(y_true, y_pred, target_names=['Нормальные', 'Атаки'], 
                                         output_dict=True)
...
class ComparisonPlotter:
    ...
    def plot_metrics_comparison(self, results_dict, title="Сравнение моделей", save_name="models_comparison.png"):
        """
    ...
    def plot_training_comparison(self, histories_dict, title="Сравнение обучения", save_name="training_comparison.png"):
        """
