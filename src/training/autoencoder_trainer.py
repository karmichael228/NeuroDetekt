...
class AutoencoderTrainer(Trainer):
    ...
    def validate_epoch(self, val_loader, desc="Валидация"):
        ...
        with torch.no_grad():
            for batch in val_bar:
                ...
                val_bar.set_postfix({
                    "образцов": len(val_errors),
                    "средн.ошибка": f"{np.mean(val_errors):.4f}"
                })
    ...
    def train_autoencoder(self, train_loader, val_loader, epochs=25, patience=8, 
                         validate_every_n_epochs=1, threshold_percentile=95,
                         model_name="autoencoder", output_dir="trials/autoencoder"):
        """
    ...
    def test_autoencoder(self, test_loader, test_labels, threshold, results_dict=None):
        ...
        evaluation = evaluate_anomaly_detection(
            ...
            threshold_percentile=int((threshold / max(test_errors)) * 100) if max(test_errors) > 0 else 95
        )
        ...
        print(f"\n📊 Дополнительная статистика:")
        print(f"   📊 Тестовых данных: {len(test_errors)} ({len(normal_errors)} норм, {len(attack_errors)} атак)")
