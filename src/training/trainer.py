...
from ..utils.helpers import TimeTracker, LossTracker, EarlyStopping, format_time
...
class Trainer:
    ...
    def train(self, train_loader, val_loader, epochs=10, verbose=True):
        ...
        for epoch in range(1, epochs + 1):
            ...
            print(f"📚 Эпоха {epoch:3d}/{epochs} | ОБУЧЕНИЕ   | Loss: {train_loss:.4f} | Accuracy: {train_acc:.3f}")
            
            # Валидация
            val_loss, val_acc = self.validate_epoch(val_loader)
            print(f"📊 Эпоха {epoch:3d}/{epochs} | ВАЛИДАЦИЯ  | Loss: {val_loss:.4f} | Accuracy: {val_acc:.3f}", end="")
            ...
            if verbose:
                ...
                print(f"⏱️  Время эпохи: {format_time(epoch_time)} | Осталось: {format_time(remaining_time)}")
