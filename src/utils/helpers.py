...
class EarlyStopping:
    ...
    def __init__(
        self, patience=7, verbose=False, delta=0, path="checkpoint.pt", restore_best_weights=True
    ):
        """
        Args:
        ...
        if self.best_score is None:
            ...
            if self.counter >= self.patience:
                ...
                if self.restore_best_weights and self.best_weights is not None:
                    ...
                    if self.verbose:
                        print(
                            f"   🔄 Восстановлены лучшие веса (val_loss: {self.val_loss_min:.6f})"
                        )
        else:
            self.best_score = score
            self.save_checkpoint(val_loss, model)
    ...
    def save_checkpoint(self, val_loss, model):
        ...
        if self.verbose:
            print(
                f"   💾 Валидационная потеря улучшилась: "
